...
#                                          num_workers=2)
train_data = torch.utils.data.DataLoader(train, batch_size=batch_size, shuffle=False, sampler=train_sampler,
                                         num_workers=2)
...
for key in patients.keys():
    ...
    val_data = torch.utils.data.DataLoader(val, batch_size=batch_size, shuffle=False, sampler=val_sampler,
                                           num_workers=2)
...
for epoch in range(epochs):
    ...
    for i, data in enumerate(train_data):
        ...
        if dice:
            # What does the module output look like?
            outputs = outputs[:, 1, :, :].unsqueeze(dim=1) # add a row
