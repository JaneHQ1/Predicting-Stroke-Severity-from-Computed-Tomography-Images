...
"""
# ROI Controller for scrolling image
class ScrollController():
    def __init__(self, fig, im_dis, vol_tran, vol_fron, vol_sagi, ax_tran, ax_fron, ax_sagi):
        self.fig = fig
        self.im_dis = im_dis
    ...
    def transverse_scroll(self, event):
        ...
        if event.button == 'up':
            self.ax_tran.index += 1
        self.im_dis.update_transverse_display(self.ax_tran.index)
    ...
    def frontal_scroll(self, event):
        ...
        if event.button == 'up':
            self.ax_fron.index += 1
        self.im_dis.update_frontal_display(self.ax_fron.index)
        self.fig.canvas.draw_idle()
    ...
    def sagittal_scroll(self, event):
        ...
        if event.button == 'up':
            self.ax_sagi.index += 1
        self.im_dis.update_sagittal_display(self.ax_sagi.index)
        self.fig.canvas.draw_idle()
