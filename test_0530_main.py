...
class MainWindow(QMainWindow):
    ...
    def load_segmentation_file(self):
        ...
        self.vol_trans = self.vol_trans + self.vol_segmen*2100
        
        #self.index_trans[0] = self.vol_trans.shape[0] // 2
        self.index_trans[0] = 135
        #self.index_gtruth[0] = self.vol_gtruth.shape[0] // 2
        self.index_gtruth[0] = 135
        #self.index_segmen[0] = self.vol_segmen.shape[0] // 2
        self.index_segmen[0] = 135
        
#        # Find unique values in array
#        print(np.unique(self.vol_trans))
#        print(np.unique(self.vol_gtruth))
#        print(np.unique(self.vol_segmen))
#        arr[arr > 255] = x # To change ndarry value
#        
#        # To check the pixel value that was used to plot image
#        pd.DataFrame(self.vol_trans[self.index_trans[-1]]).to_csv("trans_image.csv", header=None, index=None)
#        pd.DataFrame(self.vol_gtruth[self.index_gtruth[-1]]).to_csv("tgtruth_image.csv", header=None, index=None)
#        pd.DataFrame(self.vol_segmen[self.index_segmen[-1]]).to_csv("segmen_image.csv", header=None, index=None)
